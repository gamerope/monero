...
Anonymity networks can also be used with `lozzax-wallet-cli` and
`lozzax-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
must be the RPC port (typically 11112 for mainnet) instead of the p2p port:

```
HiddenServiceDir /var/lib/tor/data/monero
HiddenServicePort 11112 127.0.0.1:11112
```
