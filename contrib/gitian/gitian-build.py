...
def setup():
    ...
    os.chdir('inputs')
    if not os.path.isdir('lozzax'):
        subprocess.check_call(['git', 'clone', args.url, 'lozzax'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/lozzax/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'lozzax='+args.commit, '--url', 'lozzax='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/lozzax-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/lozzax/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/lozzax/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/lozzax-project/lozzax', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/lozzax', exist_ok=True)
    os.chdir('builder/inputs/lozzax')
    if args.pull:
